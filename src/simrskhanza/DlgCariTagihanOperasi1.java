...
public class DlgCariTagihanOperasi1 extends javax.swing.JDialog {
     ...
     * @param modal
     */
    public DlgCariTagihanOperasi1(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
...
*/
    ...
    private void BtnCari5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCari5ActionPerformed
        akses.setform("DlgCariTagihanOperasi1");
        ...
        member.addWindowListener(new WindowListener() {
            ...
            @Override
            public void windowClosed(WindowEvent e) {
                if (akses.getform().equals("DlgCariTagihanOperasi1")) {
        ...
        if (tbDokter.getSelectedRow() > -1) {
            ...
            if (kodepaket != null && !kodepaket.toString().trim().isEmpty()) {
                try {
                    ...
                    if (rs2.next()) {
                        // Cek apakah data laporan_operasi ada (tidak NULL)
                        if (rs2.getString("diagnosa_preop") != null) {
                            // Mode UPDATE - populate form dengan data existing
                            ...
                            Tindakan.setText(rs2.getString("nm_perawatan"));  // PERBAIKAN: nm_tindakan → nm_perawatan
                        } else {
                            // Mode INSERT - kosongkan form, isi field kunci saja
                            emptTeks();
                    ...
                    // Buka window dalam kedua kondisi
...
//        }
        if (!Laporan.getText().trim().equals("")) {
            try {
                // Cek apakah data laporan operasi sudah ada
                PreparedStatement cek = koneksi.prepareStatement(
                        "select no_rawat from laporan_operasi where no_rawat=? and kode_paket=? and tanggal=?");
                cek.setString(1, NoRawat.getText());
                cek.setString(2, KdTindakan.getText());
                cek.setString(3, tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString());
                ResultSet rs = cek.executeQuery();

                if (rs.next()) {
                    // Mode UPDATE - data sudah ada
                    Sequel.queryu(
                            "update laporan_operasi set diagnosa_preop='" + PreOp.getText() + "',"
                            ...
                            + "where no_rawat='" + NoRawat.getText() + "' and kode_paket='" + KdTindakan.getText() + "' and tanggal='" + tbDokter.getValueAt(tbDokter.getSelectedRow(), 0) + "'");
                    JOptionPane.showMessageDialog(null, "Laporan operasi berhasil diupdate!");
                } else {
                    // Mode INSERT - data belum ada
                    Sequel.menyimpan("laporan_operasi", "?,?,?,?,?,?,?,?", "laporan operasi", 8, new String[]{
                        NoRawat.getText(),
...
//                        tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString(),
                        ...
                        Laporan.getText(),
                        KdTindakan.getText() // Tambahan untuk kode_paket
                    });
                    JOptionPane.showMessageDialog(null, "Laporan operasi berhasil disimpan!");
                }
            ...
            JOptionPane.showMessageDialog(null, "Laporan operasi tidak boleh kosong!");
        }
...
//        emptTeks();
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgCariTagihanOperasi1 dialog = new DlgCariTagihanOperasi1(new javax.swing.JFrame(), true);
...
//                        + "from laporan_operasi where no_rawat='" + rs.getString("no_rawat") + "' or tanggal='" + rs.getString("tgl_operasi") + "'").executeQuery();
//                if (rs2.next()) {
...
//                }
                tabMode.addRow(new Object[]{
                    ...
                    "", "", "", "", "", "", "", ""
//                    tanggal,diagnosa_preop,diagnosa_postop,jaringan_dieksekusi,permintaan_pa,selesaioperasi,laporan_operasi
                });
                rs2 = koneksi.prepareStatement(
                        "select operasi.operator1, operasi.operator2, operasi.operator3, operasi.asisten_operator1,"
                        + "operasi.asisten_operator2,operasi.asisten_operator3, operasi.instrumen, operasi.dokter_anak, operasi.perawaat_resusitas, "
                ...
                while (rs2.next()) {
                    tabMode.addRow(new Object[]{"", "", "", "", no + ". " + rs2.getString("nm_perawatan"),
                        ...
                        selesaioperasi = rs2.getString("selesaioperasi"),
                        laporan_operasi = rs2.getString("laporan_operasi"),
                        kode_paket = rs2.getString("kode_paket"), //                           tanggal,diagnosa_preop,diagnosa_postop,jaringan_dieksekusi,permintaan_pa,selesaioperasi,laporan_operasi
                }
                    );
