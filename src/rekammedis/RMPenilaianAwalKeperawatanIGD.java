...
public final class RMPenilaianAwalKeperawatanIGD extends javax.swing.JDialog {
    ...
    private String masalahkeperawatanigd = "", finger = "", FileName = "", kodeberkas = "";
    private File file;
    ...
    public RMPenilaianAwalKeperawatanIGD(java.awt.Frame parent, boolean modal) {
        ...
        KeluhanUtama.setDocument(new batasInput((int) 2500).getKata(KeluhanUtama));
    ...
    private void initComponents() {
        ...
        TglAsuhan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "02-12-2025 13:26:44" }));
        ...
        TPasien1.setPreferredSize(new java.awt.Dimension(225, 23));
    ...
    private void BtnUploadActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnUploadActionPerformed
        ...
        if (tbObat.getSelectedRow() > -1) {
            ...
            // Generate nama file: AsuhanIGD_NoRawat_timestamp
            FileName = "AskepIGD_" + tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString().trim().replaceAll("/", "")
                    + "_" + timestamp;

            // Proses berurutan: PDF → JPG → Upload → Cleanup
            CreatePDFAskepIGD(FileName);
            ConvertPDFtoJPG(FileName);
            UploadJPGAskepIGD(FileName, "berkasrawat/pages/upload/");
//            HapusPDF();
        } else {
            JOptionPane.showMessageDialog(null, "Maaf, silahkan pilih data terlebih dahulu..!!!!");
            BtnBatal.requestFocus();
    ...
    private void CreatePDFAskepIGD(String FileName) {
...
//            }
            ...
            String nipPetugas = "";
            try {
                if (tbObat.getColumnCount() > 81) {
                    nipPetugas = tbObat.getValueAt(tbObat.getSelectedRow(), 79).toString();
                }
            } catch (Exception e) {
            ...
            String query = "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,if(pasien.jk='L','Laki-Laki','Perempuan') as jk,pasien.tgl_lahir,pasien.agama,bahasa_pasien.nama_bahasa,cacat_fisik.nama_cacat,penilaian_awal_keperawatan_igd.tanggal,penilaian_awal_keperawatan_igd.informasi,"
                    + "penilaian_awal_keperawatan_igd.keluhan_utama,penilaian_awal_keperawatan_igd.rpd,penilaian_awal_keperawatan_igd.rpo,penilaian_awal_keperawatan_igd.status_kehamilan,penilaian_awal_keperawatan_igd.gravida,penilaian_awal_keperawatan_igd.para,"
                    + "penilaian_awal_keperawatan_igd.abortus,penilaian_awal_keperawatan_igd.hpht,penilaian_awal_keperawatan_igd.tekanan,penilaian_awal_keperawatan_igd.pupil,penilaian_awal_keperawatan_igd.neurosensorik,penilaian_awal_keperawatan_igd.integumen,penilaian_awal_keperawatan_igd.turgor,"
                    + "penilaian_awal_keperawatan_igd.edema,penilaian_awal_keperawatan_igd.mukosa,penilaian_awal_keperawatan_igd.perdarahan,penilaian_awal_keperawatan_igd.jumlah_perdarahan,penilaian_awal_keperawatan_igd.warna_perdarahan,penilaian_awal_keperawatan_igd.intoksikasi,"
                    + "penilaian_awal_keperawatan_igd.bab,penilaian_awal_keperawatan_igd.xbab,penilaian_awal_keperawatan_igd.kbab,penilaian_awal_keperawatan_igd.wbab,penilaian_awal_keperawatan_igd.bak,penilaian_awal_keperawatan_igd.xbak,penilaian_awal_keperawatan_igd.wbak,"
                    ...
                    + "inner join bahasa_pasien on bahasa_pasien.id=pasien.bahasa_pasien "
                    + "inner join penjab on penjab.kd_pj=reg_periksa.kd_pj "
                    + "inner join cacat_fisik on cacat_fisik.id=pasien.cacat_fisik where reg_periksa.no_rawat='" + tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString() + "'";

            // Generate PDF menggunakan fungsi baru dengan query
            Valid.MyReportPDFUploadAskepIGD("rptCetakPenilaianAwalKeperawatanIGD.jasper", "report",
                    "::[ Laporan Pengkajian Awal Keperawatan IGD ]::", FileName, query, param);

        } catch (Exception e) {
            System.out.println("Error CreatePDFAskepIGD: " + e);
            e.printStackTrace();
    ...
    private void UploadJPGAskepIGD(String FileName, String docpath) {
        try {
            // Path folder tmpPDF
            ...
            kodeberkas = Sequel.cariIsi("SELECT kode FROM master_berkas_digital WHERE nama LIKE '%kep%IGD%'");
            ...
            if (kodeberkas.equals("")) {
                ...
                Sequel.menyimpan(
                        "master_berkas_digital", "'" + kodeberkas + "','Awal Keperawatan IGD'", "Master Berkas Digital"
                );
            }
            ...
            System.out.println("Upload JPG Askep IGD error: " + e);
            e.printStackTrace();
